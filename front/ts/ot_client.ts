...
/** An operator as the server writes it: an object with `ops` and `reversion`. */
...
function atomSignature(operator: Operator): string {
...
export class OtClient {
     ...
     */
    private isEcho(remote: Operator): boolean {
        ...
        if (this.isEcho(remote)) {
            ...
            lines.push(
                line(
                    ...
                    formatOperator(remote),
                    `receive=${this.reversion}`,
                    this.sendOperator ? `next send=${formatOperator(this.sendOperator)}` : "nothing left to send",
                ),
                line("    base  ", JSON.stringify(this.baseText)),
            );
